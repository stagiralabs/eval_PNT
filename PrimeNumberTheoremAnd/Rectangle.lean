...
theorem Set.left_not_mem_uIoo {a b : ℝ} : a ∉ Set.uIoo a b := by
  intro h
  simp [Set.uIoo, Set.mem_Ioo] at h
  exact absurd h.1 (not_lt_of_lt h.2)
@[target]
...
theorem Set.ne_left_of_mem_uIoo {a b c : ℝ} (hc : c ∈ Set.uIoo a b) : c ≠ a := by
  ...
  cases hc.1 with
  | inl h => exact h.ne.symm
  | inr h => cases hc.2 with
    | inl h' => linarith
    | inr h' => linarith
@[target]
theorem Set.ne_right_of_mem_uIoo {a b c : ℝ} (hc : c ∈ Set.uIoo a b) : c ≠ b := by
  ...
  | inl h =>
    -- h : c < a, need c ≠ b
    cases hc.1 with
    | inl h' => linarith  -- a < c and c < a is a contradiction
    | inr h' => exact h'.ne.symm  -- b < c gives c ≠ b
  | inr h => exact h.ne'.symm  -- c < b gives c ≠ b
@[target]
lemma left_mem_rect (z w : ℂ) : z ∈ Rectangle z w := by
  show z ∈ [[z.re, w.re]] ×ℂ [[z.im, w.im]]
  exact ⟨by simp, by simp⟩
@[target]
lemma right_mem_rect (z w : ℂ) : w ∈ Rectangle z w := by
  show w ∈ [[z.re, w.re]] ×ℂ [[z.im, w.im]]
  exact ⟨by simp, by simp⟩
@[target]
