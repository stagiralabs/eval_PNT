...
lemma square_neg (p : ℂ) (c : ℝ) : Square p (-c) = Square p c := by
  simp only [Square]
  convert Rectangle.symm using 2 <;> push_cast <;> ring
@[target]
theorem Set.left_not_mem_uIoo {a b : ℝ} : a ∉ Set.uIoo a b := by
  rcases le_or_lt a b with h | h
  · rw [Set.uIoo_of_le h]
    exact fun hh => lt_irrefl a hh.1
  · rw [Set.uIoo_of_gt h]
    exact fun hh => lt_irrefl a hh.2
@[target]
theorem Set.right_not_mem_uIoo {a b : ℝ} : b ∉ Set.uIoo a b := by
  rcases le_or_lt a b with h | h
  · rw [Set.uIoo_of_le h]
    exact fun hh => lt_irrefl b hh.2
  · rw [Set.uIoo_of_gt h]
    exact fun hh => lt_irrefl b hh.1
@[target]
